...
from data_models import CircleState, DEFAULT_MAPS, DEFAULT_ZONES


class SimpleCirclePredictor:
    """Rule-based baseline for ALGS circle zone prediction.

    It is intentionally simple, but it already accounts for:
    - map-specific priors
    - current circle position
    - circle radius
    - stage progression
    - final-stage centralization bias
    """
    ...
    def _normalize(self, scores: Dict[str, float]) -> Dict[str, float]:
        clean = {zone: max(score, 0.0) for zone, score in scores.items()}
        total = sum(clean.values()) or 1.0
        return {zone: score / total for zone, score in clean.items()}
